...
void buttonTask(void *parameter) { // 按钮任务
    while (1) {
        ...
        if (buttonState == LOW) {
            ...
            _ui_screen_change(&ui_Screen2, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_Screen2_screen_init); // 切换屏幕
        }
        
        vTaskDelay(pdMS_TO_TICKS(50)); // 使用 vTaskDelay 代替 delay
    }
}
/* Display flushing */
